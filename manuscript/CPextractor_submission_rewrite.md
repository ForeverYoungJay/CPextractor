...
Subtitle: Systems manuscript draft; independent benchmark validation pending
...
Validation status (2026-09-07): See docs/benchmark_protocol.md and docs/release_workflow.md. Historical corpus counts below are operational artifacts, not locked performance estimates. AI-assisted review must not be described as independent expert annotation. The release remains a candidate until exhaustive gold, calibration/test and ablation validation are complete.

# Abstract

Crystal plasticity parameters are dispersed across text, tables and equations, and their reuse depends on recovering material, model and loading context together with the reported values. Here we describe CPextractor, a literature-to-database system that represents these quantities as parameter claims linked to applicability and provenance objects. The current implementation combines Elsevier XML parsing, evidence selection, hierarchical schema-v6 extraction, evidence and equation linking, configurable LLM review, confidence fusion and gated database ingestion. Single-judge review is the default; committee review and deterministic rule checks are configurable experimental components. A versioned evaluation protocol separates paper selection, claim detection, field correctness, evidence grounding and database-gate validation. A fixed 50-paper review workspace supports development, calibration and held-out testing. Initial AI-assisted source review is diagnostic and does not constitute independently adjudicated expert gold. Final accuracy, calibration and ablation claims remain pending that validation. CPextractor provides an implemented framework for auditable parameter curation whose scientific reliability can be evaluated through explicit source-backed review and reproducible benchmarks.
...
The current v6 extractor directly produces hierarchical parameter_claims and supporting material, phase, process-state, condition and model objects. The parameters.registry representation is retained for compatibility with earlier code. Extractor-first postprocessing preserves newer semantic assignments while adding unit handling, metadata, evidence links and model-equation associations.
...
Papers are gathered from DOI lists, Scopus discovery or local full-text folders and decomposed into sections, tables, equations and references. The implemented acquisition path is Elsevier XML. A general PDF fallback is not established by the current release and is not included in the validated method.
...
New-schema predictions pass through extractor-first structural handling, unit processing, material/model normalization, equation linking and evidence grounding. Legacy parameter, provenance and condition rewriting is skipped for these payloads. Single-judge evaluation is the configured default. Committee review, an extractor-internal double pass and deterministic quality checks are available as separately controlled experimental options; quality checks are off by default.
...
Historical artifacts report 288 processed papers and 6,093 normalized claims. These are operational counts from an earlier corpus snapshot, not accuracy measurements for the current schema or evaluation protocol. Their exact input lineage must be recovered before reuse in a final quantitative Results section.

Raw-record and final-claim counts can differ because schemas and compatibility projections group records differently. Such counts require a fixed version and counting policy before comparison.

The presence of an evidence ID does not establish successful grounding. Source review found claims whose evidence IDs do not resolve to evidence objects, as well as values assigned from neighboring table columns. Grounding completeness must therefore be measured against resolvable source locations and independently reviewed evidence, rather than inferred from the presence of a field.

The ingestion gate uses document verdicts, confidence and review signals to determine eligibility. Historical high-confidence and admitted-claim totals describe that operational policy; they do not establish a higher accuracy among admitted records. This requires explicit paper-level acceptance labels and false-admission/false-block analysis.

The gate is intended to support selective curation. Whether its selectivity improves scientific utility remains an empirical question for the frozen calibration/test protocol.

## Document-level verdicts reveal the practical value of the layered quality-control stack

The historical gate report covers 288 papers and reports a blocked fraction of 35.4%. Unannotated papers must not be treated as correct gate negatives. The revised evaluator accepts explicit gate_should_block labels and reports unlabeled papers and missing decisions separately.

The code separates optional rule validation, evidence checks, configurable LLM review and confidence fusion. The contribution at this stage is the implemented separation and audit trail; the benefit of each component requires controlled ablation, not the existence of an internal verdict.
...
The evaluated ingestion path requires local structured evidence assets derived from XML. Handling arbitrary PDF input is outside the established release scope.
...
Claims are evaluated by the configured single-judge evaluator. An alternative committee combines evidence, normalization, consistency and meta judgments. Deterministic quality checks are separately configurable and are disabled in the default baseline. Comparison runs record these settings explicitly.
...
Figure 3 | Scientific data model and retrieval interface. The figure should show v6 `parameter_claims`, compatibility projections, evidence objects and the final hierarchy of materials, phases, process states, conditions and models. A companion panel should illustrate structured database queries, vector retrieval and evidence-constrained chatbot answering.
